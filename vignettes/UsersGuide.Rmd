...
```{r, eval=TRUE}
visualize_interactions(inter = signal)
```
...
```{r, eval=TRUE}
visualize_interactions(inter = signal,show.in=c(1,4))
```   
...
```r
> visualize_interactions(inter = signal,write.in=c(1,4))
```
...
```r
visualize_interactions(inter, show.in = NULL, write.in = NULL, write.out = FALSE, method = "default", limit = 30)
```
